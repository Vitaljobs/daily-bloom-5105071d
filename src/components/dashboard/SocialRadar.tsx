...
import { Sparkles, X, Coffee, MapPin } from "lucide-react";
...
export const SocialRadar = () => {
  const { checkedInUsers, openUsers } = useCommonGround();
        ...
        {checkedInUsers.slice(0, 8).map((user, index) => (
          <motion.button
            ...
            className={`
              ...
              ${user.status === "open" 
                ? "bg-gradient-to-br from-primary to-gold-dark text-primary-foreground avatar-ring-active" 
                : "bg-muted text-muted-foreground"
              }
              ${selectedUser?.id === user.id ? "ring-2 ring-primary ring-offset-2 ring-offset-[#1a1310]" : ""}
            `}
          >
            {user.avatar}
            {user.status === "open" && (
              <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-primary border-2 border-[#1a1310]" />
            )}
          </motion.button>
        ))}
      </div>

      {/* Selected User Card (Table Tent) */}
      <AnimatePresence>
        {selectedUser && (
          <motion.div
            ...
            className="absolute left-4 right-4 bottom-4 glass-inner p-4 z-20"
          >
            <button
            ...
            {selectedUser.status === "open" && (
              <button className="w-full btn-gold py-2 rounded-lg text-xs font-medium flex items-center justify-center gap-2">
                <Coffee className="w-3 h-3" />
                Uitnodigen voor koffie
              </button>
            )}
          </motion.div>
        )}
